document.addEventListener('DOMContentLoaded', function() {
  // alert("This works")
  // By default Use buttons to toggle between views
...
function compose_email() {
  ...
  // By default Show compose view and hide other views
...
function load_mailbox(mailbox) {
  ...
  // By default Show the mailbox and hide other views
  document.querySelector('#emails-view').style.display = 'block';
  document.querySelector('#compose-view').style.display = 'none';
  document.querySelector('#email-detail').style.display = 'none';


  // By default Show the mailbox name
  document.querySelector('#emails-view').innerHTML = `<h3>${mailbox.charAt(0).toUpperCase() + mailbox.slice(1)}</h3>`;
  // this part is added to code by me
  // Load e-mails
...
// written by Daniel, to create Elements such as 'div'
function show_email(email, mailbox){
  // first div to hold details inside of it
  ...
  timestamp.className = "col-lg-3 col-sm-3 col-sm-12";
  timestamp.innerHTML = email.timestamp;
  emailDiv.append(timestamp);

  // to create archive checkbox
  if (mailbox !=="sent") {
    const button = document.createElement('input');
    button.type = "button";
    button.className = "btn btn-sm btn btn-outline-dark";
    if (mailbox !=="inbox"){
      button.value = "Unarchive";
    }else{
      button.value = "Archive";
    }
    // button.innerHTML = "Archive html";
    emailDiv.append(button);
    button.addEventListener('click', () => change_email_archive(email.id,email.archived));
  }
    // create div to set background color and put everything inside of it
    ...
    // document.querySelector('#email-card').addEventListener('click', () => view_email(email.id));
